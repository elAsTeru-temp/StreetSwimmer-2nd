...
public class FrontCarMgr : MonoBehaviour
{
    [Header("Mgr Setting")]
    [Tooltip("トリガーと車がセットになったプレハブをアタッチ")][SerializeField] private GameObject triggerObj;
    [Header("Car Setting")]
    [Tooltip("車の速さ")] [SerializeField] private float speed = 0.06f;
    ...
    public void OrderInst(int _stageNumber)
    {
        ...
        if (_stageNumber == 2)
        {
            clone = Instantiate(triggerObj, new Vector3(0, 0, 28.1f), Quaternion.identity, space.transform);
        }
        ...
        {
            clone = Instantiate(triggerObj, new Vector3(0, 0, 28.6f), Quaternion.identity, space.transform);
        }
        else if(_stageNumber == 5)
        {
            clone = Instantiate(triggerObj, new Vector3(0, 0, 23.1f), Quaternion.identity, space.transform);
            clone = Instantiate(triggerObj, new Vector3(0, 0, 30.1f), Quaternion.identity, space.transform);
        }
