...
public class FrontCar : MonoBehaviour
{
    ...
    {
        //マネージャーを探索
        mgr = GameObject.Find("FrontCarMgr");
        //エフェクトを取得(同じ階層)
        eff = transform.parent.Find("Explosion").gameObject;
        //テキストメッシュを探索
        textMesh = transform.GetChild(0).gameObject;
        //速さを取得
        speed = mgr.GetComponent<FrontCarMgr>().GetSpeed();
        //消せる数を取得
        leftKillNum = mgr.GetComponent<FrontCarMgr>().GetKillNum();
        //テキストメッシュを更新
        UpdateTextMesh();
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (leftKillNum <= 0)
        {
            mgr.GetComponent<FrontCarMgr>().DestroyCar(this.gameObject, eff);
        }
    }
